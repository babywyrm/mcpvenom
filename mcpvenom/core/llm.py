...
def analyze_tools(tools: list[dict], model: str = "claude-sonnet-4-20250514") -> list[LLMFinding]:
    """Use Claude to analyze tool definitions for subtle security issues.

    Catches things regex can't: social engineering in descriptions, subtle
    hidden instructions, misleading parameter names, and logical attack paths.
    """
    if not tools:
        return []
    ...
    resp = _get_client().messages.create(
        ...
        messages=[{"role": "user", "content": f"Analyze these MCP tool definitions:\n\n{tools_json}"}],
    )
...
def analyze_findings(
    ...
    Takes the existing deterministic findings and asks Claude to identify
    compound attack paths, escalation routes, and risks that static chain
    detection misses.
    """
    if not findings:
        return []
...
def analyze_response(
    ...
    Catches obfuscated injection, social engineering in responses, and
    subtle data leakage that regex patterns miss.
    """
