---
name: mcpvenom-add-patterns
description: >-
  Add new regex detection patterns to mcpvenom's pattern libraries with
  test coverage. Use when adding new patterns to rules.py or probes.py,
  or when extending detection for a vulnerability class.
---

# Add mcpvenom Patterns

## Pattern Files

| File | Purpose | Used By |
|------|---------|---------|
| `mcpvenom/patterns/rules.py` | Static regex for metadata analysis | Static checks (injection, permissions, theft, etc.) |
| `mcpvenom/patterns/probes.py` | Probe payloads and response analysis | Behavioral checks (tool_probes, resource_poisoning) |
