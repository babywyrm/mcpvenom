---
name: mcpvenom-add-transport
description: >-
  Add a new transport type to mcpvenom alongside SSE, HTTP, and ToolServer.
  Use when adding support for a new MCP transport protocol like DPoP, mTLS,
  or a custom gateway.
---

# Add a New mcpvenom Transport
...
In `mcpvenom/core/session.py`, add a new class:
...
In `mcpvenom/cli.py`, add arguments:
...
In `mcpvenom/scanner.py` `scan_target()`, add label detection:
